...
int main(int argc, char **argv){
    ...
    Read(&Rhost, &Ghost, &Bhost, &M, &N, &L, "ej3.txt");
...
#include <iostream>
#include <time.h>
#include <cuda_runtime.h>blob:https://colab.research.google.com/a75a75ae-bc21-4fdc-8089-b3b77623612e
#include <stdio.h>
...
int main(int argc, char **argv){
    ...
    t1 = clock();
    funcionCPU(Rhost, Ghost, Bhost, M, N,Rhostout, Ghostout, Bhostout); // Agregar parametros!
    t2 = clock();
    ms = 1000.0 * (double)(t2 - t1) / CLOCKS_PER_SEC;
    std::cout << "Tiempo CPU: " << ms << "[ms]" << std::endl;
    Write(Rhostout, Ghostout, Bhostout, M, N, "imgCPU.txt");
